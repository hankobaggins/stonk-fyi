...
src/app/                    routes
  ...
  holders/page.tsx          one page (since 2026-09-13): the reward-coin census headline (§6g: wallets holding a reward coin,
                            category tiles), the top-10-coins holder table (§6j, since 2026-09-15), the stock-token census block (§6f: unique wallets holding any stock quote asset, issuer
                            multi-select, 24h/7d/30d, chart), then the universe table — every quote asset with HolderScan holders (+24h/7d/30d),
                            this site's on-chain wallet count (+24h, §6f census over the whole universe on paid Helius), StonkFun wallets
                            (+24h) and share. The separate hourly stock-quoted table (§6e) was retired; `holder_snapshots` is no longer written
...
13. **Not financial advice.** Keep the disclaimers that exist. The one exception to "no buy language" is the JTX affiliate button (owner's call 2026-09-10): `BuyButton` / `TradeLink` in `components/BuyButton.tsx`, URL built by `lib/jtx.ts` (`https://app.jtx.com/?mint=<mint>&ref=stonk`, `rel="sponsored"`). It sits in the nav (site-wide), the $STONK hero (large; no disclosure line under it — owner removed it 2026-09-10), the token detail header, in the full TokenTable as the "Buy" column where the Mode pill used to be (2026-09-11), and as a trailing "Trade" column in the compact TokenTable and the rewards, launches and holders tables. It is the only accent-filled element (`.btn-buy`); don't reuse that fill. `lib/jtx.ts` has no server-only imports because the Nav is a client component, so it carries its own copy of the STONK mint — keep it equal to `STONK_MINT` in `lib/api.ts`. Everything else keeps the no-buy-language rule. **Second CTA (2026-09-16): `PerpsButton`** in the same file → the owner's Imperial perps referral link (`lib/imperial.ts`, `https://www.imperial.space/refer/HANKO?to=/perps/stonk`, `rel="sponsored"`), outlined in the accent (`.btn-perps`) so the fill stays unique to `.btn-buy`. It sits beside the JTX button in the $STONK hero (stacked below sm), in the nav at xl only (the bar is too tight for two buttons between lg and xl — About collided at 1024px, Buy clipped at 700px), and at the top of the mobile menu panel below lg. STONK-only: no table-cell variant.
14. **Every render-path read has a deadline; the shell never waits for data (2026-09-16).** Supabase went unreachable (Cloudflare 522 from the project origin) and every route took ~20 s to show anything: the root layout awaited `getStonkData()` before emitting `<html>`, and nothing in the request path had a timeout. Now: `getDb()` sets supabase-js's `db.timeout` (`DB_PAGE_TIMEOUT_MS`, 4 s) with its automatic retries **off** (postgrest-js retries a failed GET 3× with 1/2/4 s backoff, which made a dead origin a fixed ~7 s per query); the cron routes call `getDb({ timeoutMs: DB_WORKER_TIMEOUT_MS })` (120 s, retries on). StonkFun / CoinGecko / Raydium / Jupiter fetches carry `AbortSignal.timeout(UPSTREAM_TIMEOUT_MS)` (10 s). The layout streams: `Nav` takes its ring as a `mark` slot and the ticker is `LiveTicker`, both behind `Suspense`, and `src/app/loading.tsx` is the route skeleton (`.skeleton` blocks, no text), so the nav/fonts/CSS paint at the TTFB (~200 ms) on every route and data streams in. Measured with a hung DB: first bytes 30–180 ms, page complete at 4 s (was 20 s). When adding a fetch or a DB read that a page awaits, give it a deadline, and never `await` data in `layout.tsx` outside a Suspense boundary.
15. **Page-side DB reads are shared-cached and breaker-guarded; the DB is sized for the worker, not for viewers (2026-09-16, same day, second incident).** The Supabase dashboard showed **1.19M API requests in 24 h (83% success) on a t3.nano**, all services unhealthy, and a restart did not hold. The worker is ~20 requests a tick (~6K/day); the rest was page renders: supabase-js queries bypass Next's fetch cache, so every render of every open tab re-ran its reads — /holders is ~30 PostgREST requests (three `quote_holder_window` RPCs over ~320 mints, ten `holderscan_series` RPCs, the census tables) plus the ~8 reads every route makes through `getStonkData`, and `LiveRefresh` re-renders each open tab every 60 s. Twenty idle /holders tabs ≈ 1.1M requests a day. Now every page-side read function is wrapped in `memoDb(name, seconds, fn)` (`lib/db.ts`: `unstable_cache`, results through JSON with Maps preserved, 60 s for STONK-live reads, 120 s for the /holders aggregates, 300 s for per-coin deltas; bypassed under `DATA_SOURCE=fixture`), so N viewers cost one query per window per Vercel region. Cache keys are the arguments: never pass a per-render value (a live price) into a memo'd function — `getPoolFlow` prices its result outside the cache for exactly that reason. A **circuit breaker** (`DB_BREAKER_MS`=30 s, on `globalThis`) makes page-side `getDb()` return null after any transport-level failure or gateway 5xx, so a dead DB costs a render nothing instead of 4 s × every read; `/api/health` → `supabase` says "breaker open" while it is. `robots.ts` disallows `/*?*` (the sortable / paged views are an unbounded render space for crawlers). **Ops decision still open:** a Nano is the free-tier instance; with the worker's write volume the project belongs on Micro or Small (Settings → Compute and Disk) — this code cut the read load, it did not add headroom.
16. **Commit hygiene** (if a repo is set up): conventional short messages; never commit `.env.local`; `next-env.d.ts` and `.next/` are generated.
...
## 6e — Holders of stock-quoted assets (`/holders`, added 2026-09-10; **retired 2026-09-13**)

**Retired:** the owner asked why /holders had two differently shaped tables; the honest answer was that this block predated the universe view (§6g) and duplicated it with a narrower scope, a different provider mix and its own cadence. The page is now one table over the whole universe (§6g) with three counts per row — HolderScan holders, this site's on-chain count of wallets holding the asset (the §6f census, extended to every universe asset on paid Helius), and StonkFun wallets. The `holders` worker step, `getHoldersTable`, `HoldersTable.tsx`, the `holder_snapshots` / `holder_windows` health checks and `getStockCoinsByMcap` are gone; `holder_snapshots` (0009, 0014's `holder_window`) stays in the DB unused (drop it when convenient). `lib/holders.ts` keeps only `STOCK_CATEGORIES` / `getStockQuoteAssets` for the census masks. The rest of this section is history.
...
**Whole-universe census (2026-09-13):** on paid Helius (`HELIUS_PLAN` set) the same run walks **every universe quote asset** (stock assets first so the issuer histogram is complete even if the budget runs out, then the rest by holder-slots), with a per-mint cap `QUOTE_CENSUS_MAX_PAGES` (150 pages = 150K accounts on paid, 100 free); a mint past the cap is stored with what was read and `truncated = true` (migration 0015) — the page shows "≥". `wallet_mint_counts` therefore holds an on-chain wallet count for ~325 assets every 6 h, read by `getMintCounts()` (lib/universe.ts) into the universe table's **On-chain** column (+24h vs the run ≥24h earlier, never for truncated mints). Sized: Σ HolderScan holders over the universe ≈ 2–3M accounts → ~2.5K pages ≈ 4–5 min at 100 ms, ~25K credits a run, ~3M a month at every 6 h; with the coin census (~3.6M) ≈ 6.6M of the Developer plan's 10M. On the free plan the run stays stock-only (a universe walk would not fit the 720 s budget at 550 ms). The stock-token de-dup block keeps its chart and issuer presets. Still open: a per-asset chart from `wallet_mint_counts`.
...
**Backfill (1).** A census figure is a de-duplicated on-chain count and nobody holds yesterday's answer, so it cannot be backdated; what can is HolderScan's own change over the coins / assets it tracks, which the worker already stores (`coin_holder_deltas` §6g, `quote_holder_deltas` §6g). Both census blocks now fill an uncovered 24h / 7d / 30d tile with that figure, marked **HS** (the universe table's convention), in holder-slots, percent against the count at the window start, with the date the census's own figure takes over in the sub-line; a footnote under each block explains the mark. `EcosystemWallets` gets `growth.d1` (only when read < 36 h ago); `WalletCensus` gets `hs: StockHsView[]` (per issuer bit: assets, tracked, holders, d1/d7/d30, summed on the page from `UniverseRow.hs`, the raw deltas now carried on every row), summed client-side over the selected issuers. `COIN_DELTAS_TOP` defaults to 600 on Advanced (was 250) so the reward-coin backfill covers more of the holder base — check `/api/health` → `coin_deltas` coverage after the next 02:15 UTC run (~60 s of paced calls inside that tick; lower it if the tick nears 300 s).

**Top coins (2).** Worker step `coin_profiles` (`lib/coin-profiles.ts` → `runCoinProfiles`): the `COIN_PROFILES_TOP` largest launched coins by StonkFun market cap (one `/tokens?sort=marketCap` page; STONK excluded, it has the home-page block), per coin three HolderScan routes — `holders?limit=1` (10 units), `holders/deltas` (20), `stats` (20: avg_time_held, retention, HHI / Gini / median) — stored in **`holderscan_snapshots`** (0014, keyed by mint; no new migration) with StonkFun price / mcap at read time and the misses in `errors`. Cadence on its own **:25 tick**: hourly, top 20 on Advanced (≈ 720K of 15M units a month; the page shows 10, so a coin climbing in usually has readings); every 12 h, top 10 on Standard (≈ 30K). `?coinprofiles=1` forces one. Coins HolderScan does not track count as failed (no row). Pruned with STONK's rows by `holder_profile_prune` (90 days).

**Page:** section "Top 10 coins by market cap: holders over time" on /holders (server component `TopCoins.tsx`), after the category tiles: rank, coin, market cap, HolderScan holders + read age, 24h / 7d / 30d (HolderScan's deltas as of that reading — backdated from the first read; 24h dropped when the reading is > 36 h old; "—" for a coin younger than the window), avg hold (n/a where HolderScan has not profiled it), StonkFun-paid holderCount live (reward coins; `.p3`), a 7-day sparkline of this site's hourly readings (`holderscan_series`, `--series-7`, "collecting" until 2 points; `.p3`), Trade. The set is live: a coin that entered the top 10 since the last read shows "next read in N min". `getCoinProfiles()` = live top 10 + newest stored row per mint (one query over the last 2 cadences, then per-mint fallback for older rows) + one `holderscan_series` RPC per mint. `DATA_SOURCE=fixture` returns a **synthetic** profile per coin (derived from the fixture's figures — not real data) so the table renders offline.

**Ops:** `/api/health` → `coin_profiles` (coins with a reading, with hold time, newest age — fails past 3× the cadence, sparkline points, top three). Tick response `notes.coin_profiles` = `18 of 20 top coins profiled, 2 not tracked by HolderScan (SYM: …), 3 without hold time (every 1h)`. Env `COIN_PROFILES_TOP`, `COIN_PROFILES_EVERY_H` override. **First-run check:** which of the top 20 HolderScan tracks (small, young coins may 404 → "next read in …" on the page for as long as that lasts) and whether `stats` answers hold time for them.

**Seen while here (not fixed, 2026-09-15):** the stock-token census (§6f) read "−127.7K (−22.6%) 24h" because its last run used up the 720 s budget after 93 of 345 assets (the whole-universe walk, §6f 2026-09-13); the stock assets are walked first so the issuer histogram should be complete, but the headline compared against a run that counted more — verify `wallet_run_meta` and either raise the budget / drop the universe walk from that run, or make the page refuse a change between runs of different coverage.
...
- Snapshot gaps (2026-09-17 check): `platform_snapshots` has holes of 2–7 h (2,462 rows where ~2,880 were due; e.g. 2026-09-16 17:00–22:00 UTC). `hourlyDeltas()` in `lib/db.ts` spreads a cumulative delta evenly over the missed hours (the catch-up reading used to draw as one $293K "hour" on /platform and the same on /launches), drops the partial current hour, and the rolling 1h/6h/24h tiles return null when the base reading sits more than two ticks (or 10% of the window) before the window start or the newest reading is >20 min old. Burn velocity and the revenue tiles were verified against the API on 2026-09-17: hourly bars since 00:00 UTC summed to $196.8K vs StonkFun's $193.9K for the day; the lifetime total equals the sum of `/revenue/history` days to within $20. `/revenue` now also carries `platformPairExcludedUsd` (~$1.12M) — not yet typed or shown.
