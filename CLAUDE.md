...
15. **Commit hygiene** (if a repo is set up): conventional short messages; never commit `.env.local`; `next-env.d.ts` and `.next/` are generated.
...
DB_PAGE_TIMEOUT_MS=4000, DB_WORKER_TIMEOUT_MS=120000, UPSTREAM_TIMEOUT_MS=10000   # render-path deadlines (rule 14)
```
