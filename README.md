...
- Price history comes from CoinGecko's public `market_chart` endpoint (coin id `stonk-3`, verified from production 2026-09-07; override with `COINGECKO_STONK_ID`; optional `COINGECKO_API_KEY` for a demo key). It is best-effort: on any failure the chart shows a placeholder instead of erroring.
