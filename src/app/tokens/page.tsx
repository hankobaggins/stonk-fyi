...
export default async function TokensPage({ searchParams }: PageProps<"/tokens">) {
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Tokens & yield" sub={<span className="num text-xs">{subtitle}</span>}>
        <div className="flex flex-col items-end gap-2">
          <form action="/tokens" className="flex gap-2">
            ...
            <input
              ...
              className="bg-surface-1 border border-border rounded-lg px-3 py-1.5 text-sm w-64 max-w-full focus:outline-none focus:border-border-strong"
            />
          </form>
          <div className="num flex flex-wrap items-center justify-end gap-2.5 text-[11px] text-muted">
            <span className="pill up">realized holder-fee APR</span>
            <span>Market snapshot: {dmy(apr.generatedAt)} · {hhmm(apr.generatedAt)}</span>
          </div>
        </div>
      </PageHeader>

      <div className="filterbar flex flex-wrap items-center gap-2">
        <span className="label text-[10px]">Sort</span>
        <div className="flex gap-1 flex-wrap">
          {SORTS.map((s) => (
            <Link key={s.v} href={href({ sort: s.v, by: undefined, dir: undefined, page: 1 })} aria-current={sort === s.v && !by}>
              {s.label}
            </Link>
          ))}
          <Link href={href({ by: "apr3", dir: "desc", mode: q.mode || "reward", page: 1 })} aria-current={by === "apr3"}>APR</Link>
        </div>
        ...
        <span className="w-px h-5 bg-border mx-1" />
        <span className="label text-[10px]">Mode</span>
        <div className="flex gap-1">
          {MODES.map((s) => (
        ...
        <span className="w-px h-5 bg-border mx-1" />
        <span className="label text-[10px]">Pair</span>
        <div className="flex gap-1 flex-wrap">
          {CATEGORIES.map((s) => (
          ...
          {q.quoteMint && (
            <Link href={href({ quoteMint: undefined, page: 1 })} aria-current>
      ...
      <Section
        ...
        action={<span className="num text-[11px] text-muted">click a column to sort · StonkFun market data · 30s · stonk.fyi payout snapshots · 5 min · Jupiter prices · 5 min</span>}
      >
        {tokens.length ? (
        ...
        <MethodStrip
          lead="APR is annualized from payouts holders actually received. It is a yield estimate, not a promised return."
          status={rewardOnPage > 0 ? <span className="text-up">{with3d} / {rewardOnPage} reward coins on this page have the full 72-hour window</span> : undefined}
          href="/about#yield"
        >
        ...
        <div className="filterbar flex items-center justify-center gap-1.5 mt-3.5 text-xs">
          <Link href={href({ page: page - 1 })} aria-disabled={page <= 1} className={page <= 1 ? "opacity-40 pointer-events-none" : ""}>← Prev</Link>
          <span className="text-secondary num px-2.5">{page} / {totalPages}</span>
          <Link href={href({ page: page + 1 })} className={page >= totalPages ? "opacity-40 pointer-events-none" : ""}>Next →</Link>
        </div>
      </Section>

      <p className="num text-xs text-muted">
        Payout history by coin on <Link href="/rewards" className="text-secondary hover:text-primary">Rewards →</Link> · definitions on <Link href="/about#yield" className="text-secondary hover:text-primary">About →</Link>
      </p>
    </div>
  );
}
