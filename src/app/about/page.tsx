...
const CADENCE: [string, string, string][] = [
  ...
  ["Holders of stock-quoted assets (24h / 7d)", "stonk.fyi hourly readings: HolderScan holder count per quote asset (GMGN for mints HolderScan does not track), StonkFun's holderCount per reward coin", "hourly"],
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div id="holders" className="scroll-mt-20" /><Section title="Holders across the ecosystem">
        <div className="text-sm text-secondary space-y-2 leading-relaxed">
          ...
          <p>The lower part of the page tracks unique holders on two sides of the tokenized-stock market on StonkFun. The quote assets are every pair StonkFun tags as xStocks, Backpack, pre-stocks or Tessera (about 80 mints); their holder count is HolderScan&apos;s figure for the mint (GMGN&apos;s for the few mints HolderScan does not track), read once an hour, which counts every wallet with a balance across all venues, not only StonkFun; a change is only ever computed between readings from the same provider. The coins are the 100 largest reward-mode coins by market cap quoted in those assets; their holder count is StonkFun&apos;s own count of reward-eligible wallets from its rewards ledger, also read hourly. Standard-mode coins have no holder figure in the API and are not listed.</p>
