...
const CADENCE: [string, string, string][] = [
  ...
  ["Runners: tokens crossing $1M–$100M market cap", "stonk.fyi crossing ledger in Postgres: every token the tick fetched plus the 100 largest by market cap, checked against StonkFun's lifetime peak each tick", "5 min"],
    ...
    <div className="grid lg:grid-cols-[200px_minmax(0,48rem)] gap-x-10 gap-y-5 lg:justify-center">
      ...
      <Section title="Holders across the ecosystem" id="holders">
        <div className="text-sm text-secondary space-y-2 leading-relaxed">
          ...
          <p>The 24h, 7d and 30d columns are the difference between the newest reading and the newest reading at or before the window start, shown once readings cover at least 80% of the window (two daily readings for 24h, about six days for 7d, 24 for 30d). Until this site&apos;s own readings cover a window, a cell or tile may borrow HolderScan&apos;s own change for the same window, marked <sup>HS</sup>: for a census figure that is HolderScan&apos;s change summed over the coins or assets it tracks, in holder-slots (a wallet holding two counts twice), so it gives the direction and rough size of the move, not the change in the de-duplicated count. Holder readings are kept for 60 days; census runs indefinitely (a few hundred rows a day).</p>
          <p>The <span className="text-primary">top 10 coins by market cap</span> table profiles the largest launched coins (STONK aside, which has its own holder base on the home page) through HolderScan every hour: its holder count, its own 24h / 7d / 30d change as of that reading — so the columns are backdated from the first read; a coin younger than a window shows none — and the average time current holders have held (n/a where HolderScan has not profiled the coin). The sparkline is this site&apos;s hourly readings over the last seven days. The set is whatever the top ten is at page load; the worker profiles the top twenty, so a coin that has just climbed in usually has readings already, and otherwise shows when the next read is due.</p>
      ...
      <Section title="Runners" id="runners">
        <div className="text-sm text-secondary space-y-2 leading-relaxed">
          <p>The <Link href="/runners" className="underline underline-offset-2 hover:text-primary">Runners page</Link> counts tokens crossing <span className="num">$1M</span>, <span className="num">$5M</span>, <span className="num">$10M</span>, <span className="num">$25M</span>, <span className="num">$50M</span> and <span className="num">$100M</span> market cap. StonkFun reports a lifetime peak market cap for every token; a peak only rises, so a token crosses a line once, and the ledger keys on (token, line): the same token crossing the same line twice cannot be counted, by construction rather than by filtering. Every tick the worker compares the peak of each token it fetched (the 100 most-traded and the 100 largest by market cap; 500 most-traded hourly; every token daily) with the lines already recorded for it and writes one row per new line with the reading time. The crossing itself happened between that reading and the site&apos;s previous reading of the token, which the ledger shows as a ± figure; a token running to $1M has volume, so it is normally in view within minutes.</p>
          <p>The 24h and 7d columns count crossings by reading time and start from the moment the ledger went live: every crossing that had already happened was seeded without a time and is never counted. A token the site has never snapshotted before, older than a day and a half, is seeded the same way when it first appears, so the daily walk cannot report old history as a runner. <span className="text-primary">Crossed</span> is cumulative (a token at $5M is also counted at $1M); <span className="text-primary">peak so far</span> groups the tokens that crossed $1M inside the window by the highest line they have reached to date, so the bands add up to the $1M+ count.</p>
          <p>The second table, <span className="text-primary">launched in the window, by peak so far</span>, is the framing of &ldquo;since date X only N tokens reached $5M&rdquo; posts: tokens launched inside the window, by the peak they have reached to date. It is shown because it is what those posts count, and labelled because it is biased against young cohorts — a token launched today has had today to run, one launched a week ago had a week. Peaks are read from the site&apos;s token index (every token daily, active tokens every 5 min), so a fresh peak can lag its read. All market caps are StonkFun&apos;s USD figures at their pricing.</p>
        </div>
      </Section>
